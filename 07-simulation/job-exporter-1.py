...
def get_stage_requirements(passed_epochs):
    if passed_epochs < 10:
        mem = [4.2, 4.8, 5.3, 5.8, 1.2]
        cpu = [2.3, 0.6, 0.7, 0.9, 0.4]
        gpu = [250, 1850, 2200, 2300, 120]
    elif passed_epochs < 50:
        mem = [4.5, 5.0, 5.5, 6.0, 1.4]
        cpu = [2.5, 0.7, 0.8, 1.0, 0.5]
        gpu = [270, 1900, 2250, 2350, 140]
    elif passed_epochs < 100:
        mem = [4.8, 5.2, 5.7, 6.2, 1.6]
        cpu = [2.7, 0.8, 0.9, 1.1, 0.6]
        gpu = [280, 1950, 2300, 2400, 160]
    elif passed_epochs < 300:
        mem = [5.0, 5.5, 6.0, 6.5, 1.8]
        cpu = [2.9, 0.9, 1.0, 1.2, 0.7]
        gpu = [300, 2000, 2350, 2450, 180]
    else:
        mem = [5.3, 5.8, 6.3, 6.8, 2.0]
        cpu = [3.0, 1.0, 1.1, 1.3, 0.8]
        gpu = [320, 2100, 2400, 2500, 200]
    
    return [{'cpu': cpu[i], 'memory': mem[i], 'gpu': gpu[i]} for i in range(5)]
...
def get_training_loss(passed_epochs, progress_percentage):
    loss_matrix = {
        10: [1.90, 1.90, 1.90, 1.90, 1.90],
        50: [1.60, 1.55, 1.50, 1.45, 1.50],
        100: [1.30, 1.25, 1.20, 1.15, 1.20],
        300: [1.00, 0.95, 0.90, 0.85, 0.90],
        500: [0.85, 0.80, 0.75, 0.70, 0.75],
    }
...
def get_training_accuracy(passed_epochs, progress_percentage):
    accuracy_matrix = {
        10: [60.0, 60.5, 61.0, 61.5, 60.5],
        50: [71.0, 72.0, 73.0, 74.0, 73.5],
        100: [77.0, 78.0, 79.0, 79.5, 80.0],
        300: [83.0, 84.0, 85.0, 86.0, 87.0],
        500: [87.0, 88.0, 89.0, 90.0, 91.0],
    }
